...
#pragma region webServer
	...
	#include <WiFi.h>
	#include <ESPmDNS.h>
	//#include <ESPAsyncWebServer-master\src\ESPAsyncWebServer.h>
	//	ESPAsyncWebServer webServer(80);
	...
	void wsrv_HandleClienthandleRoot() {
		dbg("\t\t [[handleRoot]]");
	...
	void wsrv_handle_root() {
		...
		// Intestazione fino <body>
		webpage = "<html>";
		webpage +=	"<head><title>ESP32 ROS Odom</title>";
		webpage +=		"<style>";
		webpage +=		"body { background-color: #E0E6FA; font-family: Arial, Helvetica, Sans-Serif; Color: blue;}";
		webpage +=	"</style>";
		webpage += "</head>";
		webpage += "<body>";
		...
		webpage += "<h2><br>Menu</h2>";
		webpage += "<BR><a href = '/hw' class = 'button button-blue button-bordered'><span class = 'button--inner'>Set Encoder Pins</span></a>";
		webpage += "<BR><a href = '/ros' class = 'button button-blue button-bordered'><span class = 'button--inner'>Set ROS parameters</span></a>";
		...
		sFloat = ftoa(thisSystem.g_steps2m, 2, 10); dbg2("g_steps2m: ", sFloat);
		webpage += "Parameters for odometry<BR>";

		webpage += "g_steps2m:<input type='text' value = '" + sFloat + "' name='g_steps2m'><BR>";
		//webpage += "g_steps2m:<input type = 'number' required name = 'g_steps2m' min = '0' value = '"+ftoa(thisSystem.g_steps2m,10,16) +"' step = '0.1'>";

		thisSystem.g_steps2rads = ROBOT_STEPS2RAD;
		webpage += "g_steps2rads:<input type='text' value = '" + ftoa(thisSystem.g_steps2m, 12, 20) + "' name='g_steps2rads'><BR>";

		// fine form
		webpage += "<input type='submit' value='Enter'>";// omit <input type='submit' value='Enter'> for just 'enter'"
		webpage += "</form>";
	...
	void wsrv_handle_hw()
	{
		...
		String webpage;
		webpage =  "<html>";
		webpage +=		"<head><title>HW parameters</title>";
		webpage +=			"<style>";
		webpage +=				"body { background-color: #00E0FA; font-family: Arial, Helvetica, Sans-Serif; Color: blue;}";
		webpage +=			"</style>";
		webpage +=		"</head>";
		webpage +=		"<body>";

		webpage += "<form action='http://" + IPaddress + "/hw' method='POST'>";

		webpage +=			"<h1><br>HW Parameters</h1>";//----------------------------------------------------------------------------
		webpage +=			"<p>ENCODER LEFT  A Pin: :<input  width='30' type='number' value = '" + String(thisSystem.Pin_EncLa) + "' name='Pin_EncLa'><BR>";
		webpage +=			"<p>ENCODER LEFT  B Pin: :<input  width='30' type='number' value = '" + String(thisSystem.Pin_EncLb) + "' name='Pin_EncLb'><BR>";
		webpage +=			"<p>ENCODER RIGHT A Pin: :<input  width='20' type='number' value = '" + String(thisSystem.Pin_EncRa) + "' name='Pin_EncRa'><BR>";
		webpage +=			"<p>ENCODER RIGHT B Pin: :<input  width='20' type='number' value = '" + String(thisSystem.Pin_EncRb) + "' name='Pin_EncRb'><BR>";
	...
	void wsrv_handle_ros()
	{
		...
		webpage +=		"<h1><br>ROS Parameters</h1>";
		webpage +=		"<a href = '/'>home</a>"
		webpage +=		"<form action='http://" + IPaddress + "/ros' method='POST'>";
		...
		if (webServer.args() > 0) { // Arguments were received
			for (uint8_t i = 0; i < webServer.args(); i++) {
				...
				if (webServer.argName(i) == "g_steps2rads") {
					Serial.print(" Input for [g_steps2rads] was: ");
	...
	void setup_webserver()
	{
		...
		webServer.on("/hw",		wsrv_handle_hw);
		webServer.on("/ros",	wsrv_handle_ros);
...
#pragma region ROS MACRO REGION
	...
	*/
	...
	#pragma region ROS_ODOMETRY
		...
		inline void ros_publish_odometry() {

			pub_odom.publish(&msg_odom);
			pub_encL.publish(&msg_encoderCountL);
			pub_encR.publish(&msg_encoderCountR);

			// OK si può commentare
			//char  ros_info_msg[200];
			//sprintf(ros_info_msg, "   pub_odom.topic_ %s ", pub_odom.topic_); 	ROS_INFO(ros_info_msg);
			//sprintf(ros_info_msg, "   pub_encL.topic_ %s ", pub_encL.topic_); 	ROS_INFO(ros_info_msg);
			//sprintf(ros_info_msg, "   pub_encR.topic_ %s ", pub_encR.topic_); 	ROS_INFO(ros_info_msg);
	...
	#pragma region ROS_PARAMETERS
		...
		*/
		...
		int ros_loadParameter(ros::NodeHandle* nh, const char* parName, int defaultValue=0) {
		...
		//float ros_loadParameter(ros::NodeHandle_<WiFiHardware>* nh, const char* parName, float defaultValue) {
		float ros_loadParameter(ros::NodeHandle* nh, const char* parName, float defaultValue) {
		...
		/**/
		//String ros_loadParameterStr(ros::NodeHandle_<WiFiHardware>* nh, const char* parName, String defaultValue) {
		...
		void ros_connectToServer(char* host, uint16_t httpPort =21) {
			dbgD("connecting to ");
			dbgD(host);
		...
		void ros_loadParameters() {
			...
			thisSystem.odom_x = ros_loadParameter(&nh, "x", (int)0);
			thisSystem.odom_y = ros_loadParameter(&nh, "y", (int)0);
			thisSystem.odom_r = ros_loadParameter(&nh, "r", (int)0);
		}
...
//			ros_loadParameters();
		...
		void rosCbk_initialPose(const geometry_msgs::PoseWithCovarianceStamped& initialpose) {
...
// ///////////////////////////////////////////////////////////////////////////////
	...
	//---------------------------------------------------------------------------------------
		...
		void tsk_ros(void * parameter) {
			...
			thisSystem.rosConnected = false;
			IPAddress rosIp;
			rosIp.fromString(thisSystem.ros_master_uri);
			bool blFirstTime = true;
	...
	#endif // 1
	#if 1
		void tsk_rosSpin(void * parameter) {
			const TickType_t tsk_rate = ROSRATE(5);
			dbg("\n[tsk_rosSpin START...]");

			for (;;)
			{
				nh.spinOnce();
				dbg2("Ros Spin @", millis());
		...
		void tsk_ros(void * parameter) {
			const TickType_t tsk_rate = ROSRATE(50);
			dbg("\n[tsk_ros START...]");
			...
			*/
			...
			//lancio il task che monitora i cmd/msgs in ingresso
			xTaskCreatePinnedToCore(tsk_rosSpin, "rosSpin", 3000, NULL, 3, &h_rosSpin, 1);
...
//				dbg("@#2")
				...
				ros_publish_odometry();						//pub_odom.publish(&msg_odom);
